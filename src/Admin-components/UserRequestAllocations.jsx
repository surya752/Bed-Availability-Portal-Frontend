...
const UserRequestAllocations = () => {
    ...
    const handleAllocateResource = (id) => {
        ...
        const confirmAllocation = window.confirm("Do you want to allocate this resource?");
        if (confirmAllocation) {
            ...
            AdminResourceService.allocateResource(id)
                .then((response) => {
                    ...
                    setUserRequests(prevUserRequests => prevUserRequests.map(req => req.id === id ? {...req, allocated: true} : req));
                })
                .catch(error => {
    ...
    const handleDeAllocateResource = (id) => {
        ...
        // Show confirmation popup
        const confirmDeallocation = window.confirm("Do you want to deallocate this resource?");
        if (confirmDeallocation) {
            ...
            AdminResourceService.deAllocateResource(id)
                .then((response) => {
                    ...
                    setUserRequests(prevUserRequests => prevUserRequests.map(req => req.id === id ? {...req, allocated: false} : req));
                })
                .catch((error) => {
        ...
        <div>
            ...
            <div className='row' style={{ backgroundColor: 'azure' }}>
                {userRequests.map(userRequest =>
                    <Card sx={{ minWidth: 275, backgroundColor: 'azure' }} className='col-4 m-5 jcard' key={userRequest.id}>
                        <CardContent>
