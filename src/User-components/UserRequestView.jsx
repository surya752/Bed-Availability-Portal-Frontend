import { Card, CardActions, CardContent, FormControl, FormControlLabel, IconButton, Radio, RadioGroup, TextField, Typography } from '@mui/material';
...
const UserRequestView = () => {
    ...
    const getAllUserRequest = () => {
        AdminResourceService.getAllUserRequest().then((response) => {
                ...
                {userRequests.map(userRequest =>
                    <Card sx={{ minWidth: 275, backgroundColor: 'azure' }} className='col-4 m-5 jcard'>
                        ...
                        <CardActions className='row'>
                            <div className='col-6'>
                                <Link className="btn btn-outline-success" to={`/updateUserRequest/${userRequest.id}`} ><FcExternal/>Resource</Link>
                            </div>
                        </CardActions>
                    </Card>
                )}
